...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $check = $conn->prepare("SELECT id FROM students WHERE admission_no = ? AND id != ?");
    $check->bind_param("si", $admission_no, $id);
    $check->execute();
    if ($check->get_result()->num_rows > 0) {
        $error = "Admission number already exists for another student.";
    } else {
        $update = $conn->prepare("UPDATE students SET admission_no = ?, full_name = ?, class = ?, gender = ?, parent_email = ?, parent_phone = ?, dob = ? WHERE id = ?");
        $update->bind_param("sssssssi", $admission_no, $full_name, $class, $gender, $parent_email, $parent_phone, $dob, $id);
...
<head>
    ...
    <title>Edit Student | TISM Admin</title>
...
<div class="main-content" id="main-content">
    ...
    <div class="topbar">
        ...
        <h4 class="mb-0 fw-bold text-primary">Edit Student</h4>
